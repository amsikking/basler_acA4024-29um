...
class Camera:
    '''
    Basic device adaptor for Basler acA4024-29um USB 3.0 camera. Many more
    ...
    def __init__(self,
        ...
        self._exposure_tol_us = 10                  # tolerance on exposure
    ...
    def _set_exposure_time_us(self, exposure_us):
        ...
        assert 12 <= exposure_us <= 1e7, (
            "%s: exposure_us out of range"%self.name)
        self.device.ExposureTime.Value = exposure_us
        min_us = exposure_us - self._exposure_tol_us
        max_us = exposure_us + self._exposure_tol_us
        assert min_us <= self._get_exposure_time_us() <= max_us
        if self.very_verbose:
    ...
    def _set_gain(self, gain):
        ...
        assert type(gain) is int or type(gain) is float, (
            "%s: unexpected type for exposure_us"%self.name)
        assert 0 <= gain <= 27, ("%s: gain out of range"%self.name)
    ...
    def apply_settings(
        ...
        exposure_us=None,   # 12 <= type(int) <= 10,000,000
        gain=None,          # 0 <= type(float) <= 27
...
def legalize_image_size(
    height_px='max', width_px='max', name='acA4024-29um', verbose=True):
    """returns a nearby legal image size at the *center* of the camera chip"""
    height_step, width_step = 1, 4 # device.Height.Inc, device.Width.Inc
    min_height, min_width, max_height, max_width = 6, 8, 3036, 4024
...
if __name__ == '__main__':
    ...
    # Note: fps is ~2x in "Mono8" vs "Mono12" mode (31fps spec is for "Mono8")
    print('\nMax fps test:')
    frames = 100
    camera.apply_settings(frames, 12, 0, 'min', 'max', 'auto')
    images = np.zeros(
    ...
    for i in range(num_acquisitions):
        ...
        exp_us  = np.random.randint(12, 100000)
        gain    = np.random.randint(0, 27)
